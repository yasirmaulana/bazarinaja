	...
	const plugins = [{
		...
		resolveId(id, importer, resolveOptions) {
			...
			for (const matcher of matcherStates) {
				...
				resolvedId ??= RELATIVE_IMPORT_RE.test(rawId) ? join(importer.split("?")[0], "..", rawId) : rawId;
				...
				for (const [pattern, warning, suggestions] of options.patterns) {
					...
					if (usesImport) {
						...
						if (traceEnabled) {
							...
							if (moduleGraph.has(importer)) enrichAndReport(violation, moduleGraph, resolvedImports, entries, maxTraceDepth, cwd, warnedMessages);
							else {

...
function precomputeDependencies(manifest) {
	...
	function computeDependencies(id) {
		...
		for (const depId in deps.preload) {
			...
			if (dep.preload) filteredPreload[depId] = dep;
		}
	...
	for (const moduleId of Object.keys(manifest)) computeDependencies(moduleId);
	const entrypoints = /* @__PURE__ */ new Set();
	for (const key in manifest) if (manifest[key]?.isEntry) entrypoints.add(key);
	const modules = {};
	for (const [moduleId, meta] of Object.entries(manifest)) {
		...
		if (meta.dynamicImports?.length) modules[moduleId].dynamicImports = meta.dynamicImports;
	}
	return {
		dependencies,
